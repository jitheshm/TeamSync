...
export default async (req: Request & Partial<{ user: IDecodedUser }>, res: Response) => {
    try {
        ...
        const resultObj = await projectRepository.fetchSpecificProjectDetails(req.user?.decode?.tenantId, new mongoose.Types.ObjectId(req.params.projectId), new mongoose.Types.ObjectId(req.user?.decode?.branchId as string));
