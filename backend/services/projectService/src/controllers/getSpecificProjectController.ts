...
export default async (req: Request & Partial<{ user: IDecodedUser }>, res: Response) => {
    try {
        ...
        const resultObj = await projectService.fetchSpecificProject(req.user?.decode?.tenantId as string, new mongoose.Types.ObjectId(req.params.projectId), new mongoose.Types.ObjectId(req.user?.decode?.branchId as string));
