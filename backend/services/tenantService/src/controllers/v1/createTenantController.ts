...
export default async (req: Request & Partial<{ user: IUsers }>, res: Response) => {
    try {
        ...
        console.log(bodyObj)
        const existTenant = await tenantRepository.getTenantByName(bodyObj.company_name as string)
        if(existTenant){
            return res.status(409).json({error:"Tenant already exist with this name"})
        }
